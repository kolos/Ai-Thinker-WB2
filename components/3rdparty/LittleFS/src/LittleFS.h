...
class LittleFSImpl : public FSImpl {
  public:
	...
	bool rename(const char *pathFrom, const char *pathTo) override {
		...
		if (rc != 0) {
			log_i("lfs_rename: rc=%d, from=`%s`, to=`%s`\n", rc, pathFrom, pathTo);
			return false;
		}
		return true;
	}
	...
	bool format() override {
		...
		if (_timeCallback && _tryMount()) {
			...
			if (rc != 0) {
				log_i("lfs_format, lfs_setattr 'c': rc=%d\n", rc);
				return false;
			}

			rc = lfs_setattr(&_lfs, "/", 't', &t, 8);
			if (rc != 0) {
				log_i("lfs_format, lfs_setattr 't': rc=%d\n", rc);
				return false;
			}
		...
		if (_opened && _fd) {
			...
			if (_timeCallback && (_flags & LFS_O_WRONLY)) {
				...
				if (_creation) {
					...
					if (rc < 0) {
						log_i("Unable to set creation time on '%s' to %ld\n", _name.get(), (long)_creation);
					}
				}
				...
				if (rc < 0) {
					log_i("Unable to set last write time on '%s' to %ld\n", _name.get(), (long)now);
				}
			}
		}
	}
