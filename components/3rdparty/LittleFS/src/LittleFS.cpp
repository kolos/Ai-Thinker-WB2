...
FileImplPtr LittleFSImpl::open(const char *path, OpenMode openMode, AccessMode accessMode) {
	if (!_mounted) {
		log_i("LittleFSImpl::open() called on unmounted FS\n");
		return FileImplPtr();
	}
	if (!path || !path[0]) {
		log_i("LittleFSImpl::open() called with invalid filename\n");
		return FileImplPtr();
	}
	if (!LittleFSImpl::pathValid(path)) {
		log_i("LittleFSImpl::open() called with too long filename\n");
		return FileImplPtr();
	}
...
DirImplPtr LittleFSImpl::openDir(const char *path) {
	...
	if (rc < 0) {
		log_i("LittleFSImpl::openDir: path=`%s` err=%d\n", path, rc);
		free(pathStr);
		return DirImplPtr();
	}
...
int LittleFSImpl::lfs_flash_read(
	...
	return bl_flash_read(addr, (uint8_t*)dst, size);
}
...
int LittleFSImpl::lfs_flash_prog(
    ...
    LittleFSImpl *me = reinterpret_cast<LittleFSImpl*>(c->context);
    uint32_t addr = me->_start + (block * me->_blockSize) + off;
    return bl_flash_write(addr, (uint8_t*)buffer, size);
}
...
int LittleFSImpl::lfs_flash_erase(const struct lfs_config *c, lfs_block_t block) {
	...
	return bl_flash_erase(addr, size);
}
...
#define FS_PHYS_ADDR	  0x192000
#define FS_PHYS_SIZE	  0x57000
