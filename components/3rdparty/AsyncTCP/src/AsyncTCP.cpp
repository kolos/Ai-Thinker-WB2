...
static void _tcp_dns_found(const char * name, struct ip_addr * ipaddr, void * arg) {
    ...
    e->dns.name = name;
    if (ipaddr) {
        memcpy(&e->dns.addr, ipaddr, sizeof(struct ip4_addr));
    } else {
...
void AsyncClient::_dns_found(struct ip4_addr *ipaddr){
#if LWIP_IPV4 && LWIP_IPV6
...
void AsyncClient::_s_dns_found(const char * name, struct ip4_addr * ipaddr, void * arg){
