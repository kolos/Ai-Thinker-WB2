...
File Dir::openFile(const char* mode) {
    ...
    if (!sflags(mode, om, am)) {
        blog_info("Dir::openFile: invalid mode `%s`\r\n", mode);
        return File();
    }
...
bool FS::begin() {
    if (!_impl) {
        blog_info("#error: FS: no implementation");
        return false;
    }
...
File FS::open(const char* path, const char* mode) {
    ...
    AccessMode am;
    if (!sflags(mode, om, am)) {
        blog_info("FS::open: invalid mode `%s`\r\n", mode);
        return File();
    }
